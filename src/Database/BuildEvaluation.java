...
public class BuildEvaluation {
	...
	private ArrayList<Bean> setCourseInstitutionRelation(Course course, Institution institution) throws ClassNotFoundException, SQLException{
		...
		if(courses.size() == 0 && institutions.size() == 0){
			...
			this.checkRelationAlreadyExists(courses.get(0), institutions.get(0));
	...
	private void checkRelationAlreadyExists(Course course, Institution institution) throws ClassNotFoundException, SQLException {
		boolean hasInstitution = true;
		ArrayList<Institution> institutions = course.getInstitutions();
		...
		if( institutions.size() == 0 )
			...
			for(Institution inst : institutions)
				if( inst.getId() == institution.getId() ) {
					hasInstitution = true;
					break;
				}
		}
